...
def eval_3DMatch(model, config, args):
    dset = ThreeDLOMatchTest(root='F:/gwk/PointDSC/data/3DMatch',
                             descriptor='fcgf',
...
if __name__ == '__main__':
    ...
    config_path = f'./GPINet-Net-master/snapshot/GPINet_KITTI_XXX/config.json'
    ...
    miss = model.load_state_dict(
        torch.load(f'./GPINet-Net-master/snapshot/GPINet_KITTI_XXX/models/model_best.pkl'), strict=False)
    print(miss)
    model.eval()
