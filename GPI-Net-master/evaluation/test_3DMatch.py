    ...
    with torch.no_grad():
        for i in tqdm(range(num_pair)):
            ...
            data_frame = pd.DataFrame(
                data={'scene': i,
                      'recall': float(stats[i, 0] * 100),
                      'Re': float(stats[i, 1]),
                      'Te': float(stats[i, 2]),
                      'inlier number': float(stats[i, 3]),
                      'inlier ratio': float(stats[i, 4]),
                      'output inlier number': float(stats[i, 5]),
                      'ip': float(stats[i, 6]),
                      'ir': float(stats[i, 7]),
                      'f1': float(stats[i, 8]),
                      },
            ...
            data_frame.to_csv('./GPI-Net-master/final.csv',
...
if __name__ == '__main__':
    ...
    parser.add_argument('--chosen_snapshot', default='GPINet_3DMatch_xxx', type=str, help='snapshot dir')
    ...
    config_path = './GPI-Net-master/snapshot/GPINet_3DMatch_xxx/config.json'
    #print(config_path)
    config = json.load(open(config_path, 'r'))
    config = edict(config)
    config.descriptor = 'fpfh'

    # if args.use_icp: log_filename = f'/media/lixiaohan/下载/huohu/PointDSC-master/evaluation/logs/{
    # args.chosen_snapshot}-{args.solver}-{config.descriptor}-ICP.log' else: log_filename =
    # f'/media/lixiaohan/下载/huohu/PointDSC-master/evaluation/logs/{args.chosen_snapshot}-{args.solver}-{
    # config.descriptor}.log' logging.basicConfig(level=logging.INFO, filename=log_filename, filemode='a',
    # format="") logging.getLogger().addHandler(logging.StreamHandler(sys.stdout))
    ...
    miss = model.load_state_dict(torch.load(
        './GPI-Net-master/snapshot/GPINet_3DMatch_xxx/models/model_best.pkl'),
        strict=False)  #best
    print(miss)
    # print(i)
    model.eval()
    print('generator parameters:', sum(param.numel() for param in model.parameters()))
    #print('generator parameters:', sum(mac.numel() for param in model.parameters()))
    # evaluate on the test set
    #torch.cuda.empty_cache()
    stats = eval_3DMatch(model.cuda(), config, args.use_icp)

    #input={}
    #input['xs'] = torch.randn(1000, 1, 1)
