...
def eval_KITTI(model, config, use_icp):
    dset = KITTIDataset(root='F:/gwk/datasets/KITTI',
                        split='test',
    ...
    print(allpair_average, correct_pair_average)
    # logging.info(f"*" * 40)
    # logging.info(
...
if __name__ == '__main__':
    ...
    parser.add_argument('--chosen_snapshot', default='GPINet_KITTI_XXX', type=str, help='snapshot dir')
    ...
    # if args.use_icp:
    #     log_filename = f'/media/lixiaohan/下载/huohu/PointDSC-master/evaluation/logs/{args.chosen_snapshot}-{args.solver}-ICP.log'
    # else:
    #     log_filename = f'/media/lixiaohan/下载/huohu/PointDSC-master/evaluation/logs/{args.chosen_snapshot}-{args.solver}.log'
    ...
    config_path = './GPI-Net-master/snapshot/GPINet_KITTI_XXX/config.json'
    ...
    for i in range(50, 101):
        miss = model.load_state_dict(torch.load(
            './GPINet-master/snapshot/GPINet_KITTI_XXX/models/model_best.pkl'),
                                     strict=False)
        print(miss)
        model.eval()
